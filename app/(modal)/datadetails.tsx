  ...
  if (!Array.isArray(restaurant) || restaurant.length === 0) {
    ...
    useLayoutEffect(() => {
      ...
      const renderItem: ListRenderItem<any> = ({ item, index }) => (
        <Link href={{ pathname: '/(modal)/dish', params: {...item} }} asChild>
  ...
  return (
    ...
    <>
      <ParallaxScrollView
        ...
        renderBackground={() => <Image source={{uri:restaurant.img}} style={{ height: 300, width: '100%' }} />}
        ...
        <View style={styles.detailsContainer}>
          ...
          <Text style={styles.restaurantDescription}>
            {restaurant.delivery} · {restaurant.tags.map((tag : any, index: number) => `${tag}${index < restaurant.tags.length - 1 ? ' · ' : ''}`)}
          </Text>
      ...
      <Animated.View style={[styles.stickySegments, animatedStyles]}>
        <View style={styles.segmentsShadow}>
          <ScrollView ref={scrollRef} horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.segmentScrollview}>
            {restaurant?.food.map((item: any, index:number) => (
              <TouchableOpacity
